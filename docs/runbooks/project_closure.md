...
- Archive lives on the job's **Archive** card in the portal, behind a typed confirmation that names
  exactly what moves. It relocates **eight containers** — four Smartsheet folders (Safety, Progress,
  Purchase Orders, Subcontracts) and four Box ones — into `ITS — Archive / <Job>/` and
...
> job's four Smartsheet per-job folders and its four Box folders — which carries the week sheets,
...
- **Archived took**: the job's Archive card reads "Archived. All 8 folders are filed under
  `ITS — Archive / <Job>`", those folders are visible in the Archive workspace and in Box's
  `ITS Archive`, and `ITS_Errors` has no `job_archive` rows for the job. Anything short of all
  eight is [job_archive.md](job_archive.md) Symptom 2 — it does **not** resume on its own.
