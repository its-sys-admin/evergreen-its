...
**2026-08-10 — bit live, twice, in a single session, raising severity MEDIUM → HIGH.** With
`safety_reports.publish_daemon.polling_enabled=true` on the production host, the daemon's git
cycling moved a **docs-only** session off its own feature branch mid-edit, twice: (1) a branch
was created and four docs files committed, but the commit landed on local `main` instead of the
feature branch — the daemon had switched the tree back mid-work (`git branch --show-current`
read `main` while `HEAD` already held the commit); (2) later the tree reverted entirely and an
in-progress ROADMAP `Track 6` section vanished from disk (recoverable only because it had
already been pushed in an earlier commit). Both were recovered cleanly (`git branch -f <feature>
<sha>`, then `git branch -f main origin/main`, done while standing off `main`) with nothing
permanently lost — but only because the session checked `git branch --show-current` before and
after committing. **CLAUDE.md's git-workflow claim "Docs-only edits are fine on the live tree" is
materially false while this daemon is loaded** — the daemon's `git checkout`/`git clean` do not
distinguish a doc edit from a Python-source edit, so the existing worktree-discipline carve-out
for docs undersells the actual risk. See auto-memory `live-tree-not-safe-for-docs-edits.md` for
the full narrative. This doesn't change the fix already proposed above; it's further live
evidence, now covering docs as well as code, for exactly the risk that fix targets. **Trigger
raised:** next publish-daemon hardening pass should also correct or remove the docs-only
carve-out in CLAUDE.md's git-workflow section once the daemon itself is worktree-isolated.
