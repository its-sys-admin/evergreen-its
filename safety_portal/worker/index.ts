...
app.post("/api/internal/admin/purge-job", requireAdminToken, async (c) => {
  ...
  const scheduleChunks = results[10]?.meta?.changes ?? 0;
  const scheduleRows = results[11]?.meta?.changes ?? 0;
  const schedulePreviews = results[12]?.meta?.changes ?? 0;
  const schedules = results[13]?.meta?.changes ?? 0;
  const checklistItemStates = results[14]?.meta?.changes ?? 0;
  const checklistInstances = results[15]?.meta?.changes ?? 0;
  const timeEntries = results[16]?.meta?.changes ?? 0;
  const taskAssignments = results[17]?.meta?.changes ?? 0;
  const inspections = results[18]?.meta?.changes ?? 0;
  const equipmentLocation = results[19]?.meta?.changes ?? 0;
  const submissions = results[20]?.meta?.changes ?? 0;
  const job = results[21]?.meta?.changes ?? 0;
  return c.json({
