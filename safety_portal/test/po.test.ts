...
describe("ship-to auto-fill feed", () => {
  ...
  // Teeth for the `(?![\d.])` tail anchor SPECIFICALLY. The wider tests above pass even with the
  // anchor deleted (the optional group alone handles "2026.384.1 Coker"); only these cases fail,
  // because without it a 4-digit project number or a 4-segment name still matches a 3-digit prefix.
  it("0064: the tail anchor refuses a number that only LOOKS like a prefix", async () => {
    for (const [name, expectedJobNo, expectedSite] of [
      ["2026.3845 Wider Number", "", 0], // 4-digit NNN — must not match a 3-digit prefix
      ["2026.384.1.2 Four Segments", "", 0], // not an Evergreen identifier at all
      ["2026.384.1 Coker Solar", "2026.384", 1], // the legitimate shape still parses
    ] as [string, string, number][]) {
      await env.DB.prepare("UPDATE jobs SET project_name=?1, job_no='' WHERE job_id=?2")
        .bind(name, "JOB-000017")
        .run();
      const body = await json<{ job_no: string; site_phase: number }>(
        await g(admin, "/api/po/jobs/JOB-000017/ship-to"),
      );
      expect(body.job_no, `job_no for ${JSON.stringify(name)}`).toBe(expectedJobNo);
      expect(body.site_phase, `site_phase for ${JSON.stringify(name)}`).toBe(expectedSite);
    }
  });

  it("0064: an out-of-range site in a project NAME resolves to 0, never a value the PO route rejects", async () => {
    await env.DB.prepare("UPDATE jobs SET project_name=?1, job_no='' WHERE job_id=?2")
      .bind("2026.384.99999 Over Ceiling", "JOB-000017")
      .run();
    const body = await json<{ job_no: string; site_phase: number }>(
      await g(admin, "/api/po/jobs/JOB-000017/ship-to"),
    );
    expect(body.site_phase).toBe(0); // parseDraftBody caps site_phase at 9999
  });
