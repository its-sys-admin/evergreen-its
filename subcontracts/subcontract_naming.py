...
from subcontracts import numbering
...
def _number_token(number: str) -> str:
    ...
    ``CH`` marker (e.g. ``<Job>_Subcontract_CH_2026.384.1.0.0-CO1.docx``) so a CO
    stands out in a Box/Smartsheet listing. FILE NAMES ONLY — ``sc_pdf_title`` (the
    PDF's internal ``/Title``) is deliberately unmarked; the operator scoped the
    directive to file names. Base-number names are BYTE-IDENTICAL to before this
    landed (the token only ever differs for CO-formatted numbers), so no
    previously-filed document's §47 version-on-conflict key changes; zero CO
    documents were filed before this landed, so no existing CO name shifts either."""
...
def sc_docx_filename(sc_number: str, job_name: str | None) -> str:
    ...
    (job-prefixed, matching the Safety ``<job>_<...>`` file style; a change-order number
    gains the ``CH_`` marker, see ``_number_token``). Falls back to
    ...
    number = _number_token(sc_number)
    return f"{job}_Subcontract_{number}.docx" if job else f"Subcontract {number}.docx"
...
def sc_xlsx_filename(sc_number: str, job_name: str | None) -> str:
    ...
    ``<job>_<...>`` file style; a change-order number gains the ``CH_`` marker, see
    ``_number_token``). Falls back to ``Schedule of Values <sc_number>.xlsx`` when
    the job name is empty (the number-only name)."""
    job = safety_naming.job_folder_name(job_name or "").strip()
    number = _number_token(sc_number)
    return (
        f"{job}_Schedule of Values_{number}.xlsx"
        if job
        else f"Schedule of Values {number}.xlsx"
    )
...
def sc_exhibit_filename(sc_number: str, job_name: str | None) -> str:
    ...
    matching the Safety ``<job>_<...>`` file style; a change-order number gains the ``CH_``
    marker, see ``_number_token``). Falls back to ``Exhibit A <sc_number>.docx``
    when the job name is empty (the number-only name)."""
    job = safety_naming.job_folder_name(job_name or "").strip()
    number = _number_token(sc_number)
    return f"{job}_Exhibit A_{number}.docx" if job else f"Exhibit A {number}.docx"
...
def sc_package_zip_filename(sc_number: str, job_name: str | None) -> str:
    ...
    attachment content-type (``application/zip``). A change-order number gains the ``CH_``
    marker (see ``_number_token``)."""
    job = safety_naming.job_folder_name(job_name or "").strip()
    number = _number_token(sc_number)
    return (
        f"{job}_Subcontract Package_{number}.zip"
        if job
        else f"Subcontract Package {number}.zip"
    )
...
def sc_pdf_filename(sc_number: str, job_name: str | None) -> str:
    ...
    matching the Safety ``<job>_<...>.pdf`` file style; a change-order number gains the
    ``CH_`` marker, see ``_number_token``). Falls back to
    ``Subcontract <sc_number>.pdf`` when the job name is empty (the number-only name)."""
    job = safety_naming.job_folder_name(job_name or "").strip()
    number = _number_token(sc_number)
    return f"{job}_Subcontract_{number}.pdf" if job else f"Subcontract {number}.pdf"
