"""One-shot migration: seed the `field_ops.schedule_poll.*` ITS_Config rows
(ADR-0006 PR-3 — the job-schedule importer daemon).

Companion to `field_ops/schedule_poll.py`. Run once at PR landing; safe to re-run
(per-row idempotency-guarded on Setting+Workstream — the seed_manifest_config.py
pattern). Enrolled in `standup.SEEDER_STAGE_SCRIPTS` in the SAME change — a seeder
that exists on disk but that no stand-up stage runs is exactly how the manifest rows
went missing on the 2026-08-10 rebuilt tenant.
...
What it seeds (2 rows, workstream `field_ops`):
    ...
    field_ops.schedule_poll.polling_enabled       = false  (the ONE daemon gate — dark)
    field_ops.schedule_poll.poll_interval_seconds = 120    (install-time cadence)
...
Run from `~/its` with the venv activated:
    ...
    python3 scripts/migrations/seed_schedule_config.py
...
CONFIG_ROWS: list[dict[str, Any]] = [
    {
        "Setting": "field_ops.schedule_poll.polling_enabled",
        "Workstream": WORKSTREAM,
        "Value": "false",
        "Description": (
            "Gate for the job-schedule importer daemon (schedule_poll, ADR-0006 "
            "PR-3): pull office-uploaded project-schedule PDFs from the Worker pool, "
            "schedule:v1 HMAC- and digest-verify, §34 doc-screen, OCR the document "
            "inside the killable sandbox child (Quartz render + Apple Vision — "
            "LOCAL, no cloud AI), reconstruct + parse the table, file the ORIGINAL "
            "document to Box <job>/Schedules/, and post a reviewable grid for the "
            "validate screen. It NEVER commits a task by itself — the parser "
            "proposes, the human disposes (the validate screen's side-by-side "
            "preview is the only fidelity control). Turning this ON is a capability "
            "activation (§44): confirm with Seth. Preconditions live in "
            "docs/runbooks/schedule_import_path.md (Go-live), not in this cell — "
    ...
    {
        "Setting": "field_ops.schedule_poll.poll_interval_seconds",
        "Workstream": WORKSTREAM,
        "Value": "120",
        "Description": (
            "Integer seconds between schedule_poll cycles. Read at INSTALL time by "
            ...
            "the next `install.sh load org.solutionsmith.its.schedule-poll`, not "
            "hot). Default 120s, matching manifest-poll: schedule exports are an "
            "office upload trickle, and the work per document (screen, sandboxed "
            "render + OCR, Box file) is the same shape. If you raise this "
            "materially, widen the schedule_poll marker window in "
            "scripts/watchdog.py too."
        ),
    },
]
...
def seed_config_rows() -> list[tuple[str, str]]:
    """Seed the schedule_poll rows. Idempotent per row.
...
def main() -> int:
    ...
    print(f"[info] Seeding {len(CONFIG_ROWS)} rows (schedule_poll: gate false + "
          f"interval; the Worker base URL, Box root and ClamAV posture reuse the "
          f"existing safety_reports / po_materials rows)")
    print()
