...
| `po_materials/` | The Purchase-Order workstream — a deterministic (no-AI) direct-PO-to-vendor pipeline: `po_generate` (integer-cents render, Worker-parity assert), `po_poll` (pull daemon), `po_send`/`_poll` (F22 send half), plus the §50 **config actuator** (`config_actuator`/`config_apply`) that commits + deploys approved workstream-config edits. Every stage carries its own `po_materials.*` ITS_Config gate, generation gated separately from send — read ITS_Config for live state, never this table. |
| `subcontracts/` | Deterministic subcontract-package generation (ADR-0003, no AI) — `subcontract_generate` (SOV guard → §50 legal gate → token fill), `subcontract_docx` (editable `.docx`/`.xlsx`, not PDF), `subcontract_poll` (pull daemon), a WSR review twin. Both halves are built — `subcontract_send`/`_poll` (SC-S4) is the F22 send half — each behind its own `subcontracts.*` ITS_Config gate; read ITS_Config for live state, never this table. |
| `operator_dashboard/` | The WS2 operator dashboard — a localhost-only FastAPI app (`python -m operator_dashboard` @127.0.0.1:8484, Tailscale-exposed): loginless read-only observability panels + a PIN-gated ACT surface (ITS_Config editor, secret rotation). Writes only ITS_Config; never deploys or sends. Fail-closed: the ACT surface stays inert until `ITS_OPERATOR_PIN` is set in the Keychain. |
