...
def test_parse_rejects_change_order_suffix() -> None:
    ...
    parse_sc_number handles BASE family numbers only. Change-order derivation is a
...
def test_change_order_parts_grammar() -> None:
    """The `-CO<digits>` split (twin of tests/test_po_numbering.py — the lanes share
    no module): parent from the SIGNED number string, None on any malformation (a
    malformed suffix renders NO clause / NO CH file-name marker rather than a wrong
    one)."""
    ...
    assert parts("2026.384.1.0.0") is None      # a normal SC never matches
